...
class textGenerator(object):
    ...
    def getMessageOnRecurrence(self, variables={}, calledBy=''):
        ...
        n = random.random()
        if n < 0.45:
            category = 'magicEightBallPositive'
        elif n < 0.9:
            category = 'magicEightBallNegative'
        else:
            category = 'magicEightBallNeutral'
        message =  self.torolib.buildMessage(texts = self.texts, textIndices = self.textIndices, tags=[category], variables=variables)
        if message:
